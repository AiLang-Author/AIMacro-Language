...
Gates (host): curated **25/25**, matrix **75/75/75**, fizzbuzz **215153**, Hash **922**.

## Probe (py3.13 stdlib)
| Module | compile |
|--------|---------|
| heapq, colorsys, keyword, quopri, bisect | **ok** |
| copy | **issubclass** (rc=1) |
| statistics | was Expected ')' (method return listcomp) → now **Unexpected token** (rc=139) |
| types | Unexpected token then SIGSEGV |

## Ladder
1. ~~map~~ 2. ~~iter~~ 3. ~~next~~ 4. ~~h_append_bound~~ 5. ~~copier_dispatch~~ 6. ~~return_listcomp_method~~
7. Next leftover: `issubclass` (copy.py rc=1) or statistics/types Unexpected token.
